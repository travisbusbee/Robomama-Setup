...
def edge_find(kp, xStart, yStart, zStart, step1, step2, backstep, dwell, speed1, speed2, tolerance,  direction='+x', find_mid = 'Yes', axis='D'):
    ...
    while abs(value) < tolerance:
        ...
        if value is None:
            value = tolerance + 1
    coordinate = g.get_axis_pos(axis=axis)
    return coordinate
...
def run_alignment_script(Substrate_xRef, Substrate_yRef, axis = 'D'):
   ...
   (profilometer_x_groove_new, profilometer_y_groove_new) = xy_align_profilometer(axis = axis)
   
   #Profile z_ref location
   (ref_prof_position) = get_z_ref(x, y, zStart, axis = axis, find_mid = 'Yes')
   
   #find substrate boundaries
   (substrate_left_x, substrate_top_y), (substrate_right_x, substrate_bottom_y), (prof_substrate_start)=  identify_substrate_location(xStart, yStart, zStart = ref_axis_pos, axis = axis, edges = 'All')
   
   #move to substrate reference and obtain profilometer ref
   (substrate_z_pos) = get_substrate_ref(xRef=Substrate_xRef, yRef=Substrate_yRef, zStart=prof_substrate_start, axis = axis)
